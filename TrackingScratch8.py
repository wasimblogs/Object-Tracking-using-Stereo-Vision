...
def go():
    ...
    while True:
        ...
        retL, imageL = left.read()
        rectangles = te.blobs(frame, DILATE=3)
        start = 0

        l = len(rectangles)
        if l == 0: noObjectFrameCount += 1
        print "No. of objects", l
        sudden, suddenChangeScore = te.suddenChange(frame, oldframe)
        oldframe = frame

        # IF THERE ARE SUDDEN CHANGES IN FRAMES DO NOT USE THEM FOR OBJECT MODELLING
        objectModels(imageL,frame, rectangles)

        sum += len(rectangles)
        # except:
        #     print "POSSIBLY END OF FILE"
        #     result.release()
        #     break
